...
class ChatClient(Frame):
  ...
  def initUI(self):
	...
	bottomLabel = Label(parentFrame, text="P2P Chatting Application which uses message verification using ECDSA")
  ...
  def listenClients(self):
	while 1:
	  ...
	  clientsoc.send(self.pubKey.to_string())
  ...
  def handleClientMessages(self, clientsoc, clientaddr):
	flag=0
	while (flag==0):
		data=clientsoc.recv(48)
		if (len(data)==48):
			self.veriKey= VerifyingKey.from_string(data,curve=NIST192p)
			print "len of verikey:" + str(len(self.veriKey.to_string()))
			flag=1
	while True:
		try:
			...
			msg= data[48:]
			try:
				self.veriKey.verify(sign,msg)
				self.addChat("%s:%s" % clientaddr, msg)
			except:
				self.setStatus("Signatures not verified. Exit the chat")
		except:
			break
  ...
  def handleSendChat(self):
	...
	if msg == '':
		return
	msg = self.priKey.sign(msg)+ msg  
	self.addChat("me", data)
